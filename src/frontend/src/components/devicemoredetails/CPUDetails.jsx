...
import { Monitor, Cpu, Calendar, CalendarDays, ChevronDown, AlertCircle, RefreshCw, Clock } from 'lucide-react';
...
const CpuDetails = ({ isDarkMode = false }) => {
  ...
  const cpuModel = device?.device?.cpu?.[0]?.model || 'Unknown CPU';
  const p_cores = device?.device?.cpu?.[0]?.p_cores || 'unknown';
  const l_cores = device?.device?.cpu?.[0]?.l_cores || 'unknown';
  const cpuThreads = device?.device?.cpu?.[0]?.threads || 'unknown';
  ...
  // SEPARATE GRANULARITY STATES: temp for UI selection, applied for actual data display
  ...
  } = useGetCpuMinutelyStatsQuery(id, { skip: customRangeApplied || !id });
  ...
  } = useGetCpuHourlyStatsQuery(id, { skip: customRangeApplied || !id });
  ...
  } = useGetCpuDailyStatsQuery(id, { skip: customRangeApplied || !id });
  ...
  } = useGetCpuWeeklyStatsQuery(id, { skip: customRangeApplied || !id });
  ...
  } = useGetCpuMonthlyStatsQuery(id, { skip: customRangeApplied || !id });
  
  // Custom range mutation with enhanced error handling
  ...
  const getPerformanceIndicator = (value) => {
    const performance = getPerformanceLevel(value, 'cpu'); 
    return (
  ...
  // Data transforms for charts with error handling
  ...
  const customChartData = useMemo(() => {
    console.log('Processing custom CPU chart data:', { 
      customRangeApplied, 
      customData, 
      customError,
      isCustomLoading,
    ...
    try {
      ...
      console.log('Extracted actual CPU data:', {
        actualData, 
        detectedGranularity,
        appliedGranularity,
      ...
      console.error('Error transforming custom CPU data:', error, { customData });
    }
  ...
  // Granularity change handler - only updates temp state
  ...
  const applyCustomRange = async () => {
    ...
    try {
      ...
      console.log('Applying custom CPU range:', apiParams);

      resetCustomStats();
      
      const result = await getCustomStats(apiParams).unwrap();
      
      console.log('Custom CPU stats API result:', result);
      ...
      console.error('Error applying custom CPU range:', error);
      
      if (error?.status === 404) {
        setDateError(`No CPU data available for the selected ${tempGranularity} range`);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              {cpuModel} • {p_cores} p_cores • {l_cores} l_cores
            </p>
          </div>
        </div>
        ...
        <div className="text-center py-12">
          <div className="animate-pulse flex flex-col items-center">
            ...
            <div className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">
              Loading {customRangeApplied ? `custom range (${appliedGranularity})` : 'live'} CPU statistics...
            </div>
          ...
          {customRangeApplied ? (
                ...
                <div className={cardClass(isDarkMode)}>
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-md font-semibold flex items-center">
                      <span>Custom Range CPU Utilization ({appliedGranularity})</span>
                    </h4>
                  ...
                  {customChartData.length === 0 && !isCustomLoading && (
                    <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-sm font-medium mb-1">No CPU data available</div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-6">
                <div className={cardClass(isDarkMode)}>
                  <h4 className="text-md font-semibold mb-2 flex items-center justify-between">
                    <span>Real-time CPU Utilization</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className={cardClass(isDarkMode)}>
                  <h4 className="text-md font-semibold mb-2 flex items-center justify-between">
                    <span>Hourly CPU Utilization</span>
                ...
                <div className={cardClass(isDarkMode)}>
                  <h4 className="text-md font-semibold mb-2 flex items-center justify-between">
                    <span>Daily CPU Utilization</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className={cardClass(isDarkMode)}>
                  <h4 className="text-md font-semibold mb-2 flex items-center justify-between">
                    <span>Weekly CPU Utilization</span>
                ...
                <div className={cardClass(isDarkMode)}>
                  <h4 className="text-md font-semibold mb-2 flex items-center justify-between">
                    <span>Monthly CPU Utilization</span>
