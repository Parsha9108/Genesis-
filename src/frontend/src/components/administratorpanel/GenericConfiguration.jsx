...
const GenericConfiguration = ({
  ...
  const licenseKeys = ['license.key', 'license.name', 'license.email', 'license.organization'];
  ...
  try {
    ...
    console.log('Save Config Info:', {
      configType,
      isUpdate,
      changedFieldsCount,
      method: configType === 'license' ? (isUpdate ? 'PUT' : 'POST') : 'PATCH',
      payload
    });
    ...
    toast.success(response?.message || (isUpdate ? 'Configuration updated successfully' : 'Configuration saved successfully'));
  ...
  const renderField = (field) => {
    ...
    const inputClasses = `w-full px-3 py-2 rounded-lg border ${
      ...
      !hasUpdatePermission && !isReadOnlyField ? 'opacity-60 cursor-not-allowed' : ''
    }`;
    ...
    const helpTextClasses = `text-xs mt-1 ${
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 ml-2 transition-transform duration-200 ${
                    dropdownStates[field.key] ? 'rotate-180' : 'rotate-0'
                  }`}
                />
              </button>
