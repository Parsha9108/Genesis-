...
const DeviceInventory = ({ isDarkMode }) => {
  ...
  // Query optimized to reduce rerenders during polling
  const { data, isLoading, error, refetch } = useGetDeviceDetailsByIdQuery(id, {
    pollingInterval: 5000,
    selectFromResult: (result) => {
      const device = result.data?.device ?? emptyObj;
      return {
        data: result.data
          ? {
              device: {
                ...device,
                cpu: device.cpu ?? emptyArr,
                gpu: device.gpu ?? emptyArr,
                memory: device.memory ?? emptyArr,
                nic: device.nic ?? emptyArr,
                storage: device.storage ?? emptyArr,
              },
            }
          : undefined,
  ...
  // Stable scroll function - no dependencies
  ...
  // Single card click handler - simplified dependencies
  ...
  // Auto-scroll effect when coming from flag navigation
  useEffect(() => {
  ...
  // Stable helper functions with useMemo
  ...
  const hasValidGateway = useMemo(() => (nic) => {
    ...
    return nic.port.some(
      (port) =>
        port.ip &&
        Array.isArray(port.ip) &&
        port.ip.some((ip) => {
  ...
  const InfoRow = useMemo(() => ({ label, value, highlight = false }) => (
    <div className="flex justify-between items-center py-2 border-b border-gray-200/20 last:border-b-0">
      ...
      <span
        className={`text-xs sm:text-sm font-semibold truncate max-w-[60%] text-right ${
  ...
  // Stable ComponentCard with memo
  ...
  return (
    <div className="min-h-[85vh]">
      {/* Fixed Device Inventory Header - Responsive */}
      <div 
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div
                  className={`px-2 py-1 rounded-full text-xs font-medium border shadow-sm whitespace-nowrap ${
                    isDarkMode ? 'bg-[#6366f1]/20 text-blue-300 border-blue-700/30' : 'bg-blue-100 text-blue-700 border-blue-200'
                  }`}
                >
                  {device.device?.dev_phy_vm || 'Virtual Machine'}
                </div>
                <div
                  className={`px-2 py-1 rounded-full text-xs font-medium border shadow-sm whitespace-nowrap ${
                    isDarkMode ? 'bg-gray-700/50 text-gray-300 border-gray-600/30' : 'bg-gray-100 text-gray-700 border-gray-200'
                  }`}
                >
                  {device.device?.make || 'QEMU'}
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Content area with adjusted top margin - Responsive padding */}
      <div className="pt-10 sm:pt-24 space-y-4 sm:space-y-5 px-3 sm:px-0">
        
        {/* System Summary Card - Responsive grid */}
        <div
          className={`rounded-xl p-4 sm:p-6 border ${
            isDarkMode ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700' : 'bg-gradient-to-br from-blue-50 to-indigo-100 border-blue-200'
          }`}
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
            <div
              ...
              className={`rounded-lg p-3 sm:p-4 text-center shadow-sm border cursor-pointer transition-all duration-200 hover:shadow-lg ${
            ...
            >
              <Cpu className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 mx-auto mb-2" />
              <p 
                className="text-xl sm:text-2xl font-bold"
                style={{ color: isDarkMode ? '#FFFFFF' : '#1F2937' }}
              >
                {device.device?.cpu?.[0]?.p_cores || 0}
              </p>
              <p 
                className="text-xs sm:text-sm"
                style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280' }}
            ...
            >
              <MemoryStick className="w-5 h-5 sm:w-6 sm:h-6 text-green-600 mx-auto mb-2" />
              <p 
                className="text-xl sm:text-2xl font-bold"
                style={{ color: isDarkMode ? '#FFFFFF' : '#1F2937' }}
              >
                {(device.device?.memory?.[0]?.size || '0').toString().replace(' GB', '')}
              </p>
              <p 
                className="text-xs sm:text-sm"
                style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280' }}
            ...
            >
              <HardDrive className="w-5 h-5 sm:w-6 sm:h-6 text-orange-600 mx-auto mb-2" />
              <p 
                className="text-xl sm:text-2xl font-bold"
                style={{ color: isDarkMode ? '#FFFFFF' : '#1F2937' }}
              >
                {device.device?.storage?.length || 0}
              </p>
              <p 
                className="text-xs sm:text-sm"
                style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280' }}
            ...
            >
              <Gpu className="w-5 h-5 sm:w-6 sm:h-6 text-purple-600 mx-auto mb-2" />
              <p 
                className="text-xl sm:text-2xl font-bold"
                style={{ color: isDarkMode ? '#FFFFFF' : '#1F2937' }}
              >
                {device.device?.gpu?.length || 0}
              </p>
              <p 
                className="text-xs sm:text-sm"
                style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280' }}
              >
                GPU{(device.device?.gpu?.length || 0) !== 1 ? 's' : ''}
              </p>
            </div>
            ...
            >
              <Network className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 mx-auto mb-2" />
              <p 
                className="text-xl sm:text-2xl font-bold"
                style={{ color: isDarkMode ? '#FFFFFF' : '#1F2937' }}
              >
                {device.device?.nic?.length || 0}
              </p>
              <p 
                className="text-xs sm:text-sm"
                style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280' }}
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* CPU Section */}
          {device?.device?.cpu && device.device.cpu.length > 0 && (
            <ComponentCard ref={cpuRef} title="Processor" icon={Cpu} sectionKey="cpu" count={device.device.cpu.length}>
              <div className={`grid gap-4 sm:gap-6 ${getGridCols(device.device.cpu.length)}`}>
                {device.device.cpu.map((cpu, index) => (
                  <div key={cpu.uuid || `cpu-${device.device_id || id}-${index}`} className={`rounded-lg p-4 sm:p-5 border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
          ...
          {/* Memory Section */}
          {device?.device?.memory && device.device.memory.length > 0 && (
            <ComponentCard ref={memoryRef} title="Memory" icon={MemoryStick} sectionKey="memory" count={device.device.memory.length}>
              <div className={`grid gap-4 sm:gap-6 ${getGridCols(device.device.memory.length)}`}>
                {device.device.memory.map((memory, index) => (
                  <div key={memory.uuid || `mem-${device.device_id || id}-${index}`} className={`rounded-lg p-4 sm:p-5 border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          ...
          {/* Storage Section */}
          {device?.device?.storage && device.device.storage.length > 0 && (
            <ComponentCard ref={storageRef} title="Storage Devices" icon={HardDrive} sectionKey="storage" count={device.device.storage.length}>
              <div className={`grid gap-4 sm:gap-6 ${getGridCols(device.device.storage.length)}`}>
                {device.device.storage.map((storage, index) => {
                  const isFlagged = storage.is_flagged === true;

                  return (
                    <div
                      key={storage.uuid || `stor-${device.device_id || id}-${index}`}
          ...
          {/* GPU Section */}
          {device?.device?.gpu && device.device.gpu.length > 0 && (
            <ComponentCard ref={gpuRef} title="Graphics" icon={Gpu} sectionKey="gpu" count={device.device.gpu.length}>
              <div className={`grid gap-4 sm:gap-6 ${getGridCols(device.device.gpu.length)}`}>
                {device.device.gpu.map((gpu, index) => (
                  <div key={gpu.uuid || `gpu-${device.device_id || id}-${index}`} className={`rounded-lg p-4 sm:p-5 border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          ...
          {/* Network Section - Responsive port cards */}
          {device?.device?.nic && device.device.nic.length > 0 && (
            <ComponentCard ref={nicRef} title="Network Interfaces" icon={Network} sectionKey="nic" count={device.device.nic.length}>
              <div className={`grid gap-4 sm:gap-6 ${getGridCols(device.device.nic.length)}`}>
                {device.device.nic.map((nic, index) => {
                  ...
                  return (
                    <div
                      key={nic.uuid || `nic-${device.device_id || id}-${index}`}
                      className={`rounded-lg p-4 sm:p-5 border relative transition-opacity ${
                        isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'
                              ...
                              return (
                                <div
                                  ...
                                  className={`rounded-lg border p-3 sm:p-4 transition-all duration-300 ${
                                    ...
                                    isFlaggedPort 
                                      ? 'border-red-500 bg-red-50 dark:bg-red-900/30 shadow-red-100 dark:shadow-red-900/20' 
                                      : 'hover:shadow-md'
                                  }`}
                                >
                                  {/* Port Header */}
                                  <div className="flex items-center justify-between mb-2 sm:mb-3">
                                    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
                                      <div className={`p-1 sm:p-1.5 rounded-md flex-shrink-0 ${
                                        isFlaggedPort ? 'bg-red-600' : 'bg-[#6366f1]'
                                      }`}>
                                    ...
                                    {/* Port Status Badge */}
                                    {isFlaggedPort ? (
                                      <span className="px-1.5 sm:px-2 py-0.5 rounded-full text-xs font-semibold bg-red-600 text-white whitespace-nowrap flex-shrink-0">
                                        Flagged
                                      </span>
                                    ) : (
                                      <span className="px-1.5 sm:px-2 py-0.5 rounded-full text-xs font-semibold bg-green-600 text-white whitespace-nowrap flex-shrink-0">
                                        Active
                                      </span>
                                    )}
                                  </div>
