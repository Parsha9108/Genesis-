...
ROLE_DEFAULT_PERMISSIONS = {
    ...
    # Admin: omit here to grant all via seeding
}
def update_web_user(request, id):
    ...
    is_admin_updating_others = user.role.lower() != 'admin'  
    ...
    data = request.data
    serializer = UserUpdateSerializer(user, data, partial=True)
    ...
    with transaction.atomic():
        ...
        if role_changed:
            ...
            if new_role == 'admin':
                ...
                default_names = ROLE_DEFAULT_PERMISSIONS.get(new_role, [])
                perms = Permission.objects.filter(name__in=default_names)

            # Create UserPermission entries
            bulk = [
                UserPermission(user=user, permission=perm, is_active=True)
                for perm in perms
            ]
    ...
    return Response(
        {
            ...
            'update_type': 'self_update' if is_self_update else 'admin_update'
        },
