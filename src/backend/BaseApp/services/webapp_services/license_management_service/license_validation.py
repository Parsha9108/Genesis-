...
from BaseApp.utils import activate_license_if_needed,validate_db_binding,validate_deployment
...
def validate_agent_request():
    ...
    ok, msg = validate_deployment(payload)
    if not ok:
        return False, msg

    activate_license_if_needed()

    ok, msg = validate_db_binding()
    if not ok:
        return False, msg

    expiry = datetime.fromisoformat(
        payload["expiry"].replace("Z", "+00:00")
    )
    if datetime.now(timezone.utc) > expiry:
        return False, "License expired"
